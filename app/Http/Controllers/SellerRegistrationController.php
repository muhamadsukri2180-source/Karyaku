...
class SellerRegistrationController extends Controller
{
    public function create(Request $request)
    {
        ...
        if (($user->role?->role_name ?? null) === 'penjual' || 
            IdentityVerification::where('user_id', $user->id_user)->where('status', 'pending')->exists()) {
        ...
        if (!$selectedMembershipId || !$memberships->contains('id_membership', (int) $selectedMembershipId)) {
            $selectedMembershipId = $memberships->first()->id_membership ?? null;
        }
        ...
        return view('pembeli.daftar-penjual', compact('user', 'memberships', 'selectedMembershipId', 'banks', 'paymentMethods'));
    }
    ...
    {
        $registration = IdentityVerification::where('user_id', Auth::id())
