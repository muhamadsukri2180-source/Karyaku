...
<head>
    ...
    <title>Karyaku - Verifikasi Pembayaran Membership</title>
    <!-- Tailwind CSS -->
    ...
    <div class="flex min-h-screen relative">
        ...
        <aside id="sidebar" class="w-[260px] bg-gradient-to-b from-skyDeep via-skyHover to-sky text-white flex flex-col shrink-0 border-r border-sky-400/20 shadow-2xl fixed lg:sticky top-0 h-screen z-50 closed lg:translate-x-0">
            ...
            <nav class="flex-1 px-4 space-y-1.5 text-[13px] font-semibold text-sky-100 overflow-y-auto pb-4">
                ...
                <a href="{{ route('verifikator.pembayaran') }}" class="w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl active-menu transition-all group">
                    <div class="flex items-center gap-3">
                        <i class="fa-solid fa-receipt w-4 text-center text-white"></i><span>Verifikasi Pembayaran</span>
                    </div>
                </a>
        ...
        <main class="flex-1 flex flex-col min-w-0 w-full">
            <header class="bg-white/70 backdrop-blur-xl border-b border-sky-200 px-6 py-4 flex items-center justify-between sticky top-0 z-30 shadow-sm">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h2 class="text-xl sm:text-2xl font-extrabold tracking-tight font-display text-slate-900">Verifikasi Pembayaran Membership</h2>
                        <p class="text-[11px] sm:text-xs text-slate-600 font-semibold mt-0.5">Konfirmasi kecocokan resi transfer pembayaran paket membership.</p>
                    </div>
                </div>
            </header>
            ...
            <div class="p-6 sm:p-8 space-y-6">
                ...
                <div class="flex border-b border-sky-200 gap-4">
                    <a href="{{ route('verifikator.pembayaran', ['tab' => 'pending']) }}" class="pb-3 px-2 text-sm font-bold border-b-2 transition-all {{ $tab === 'pending' ? 'border-sky text-sky-600' : 'border-transparent text-slate-400 hover:text-slate-600' }}">
                        <i class="fa-solid fa-clock mr-1.5"></i> Pending Transfer
                    </a>
                    <a href="{{ route('verifikator.pembayaran', ['tab' => 'history']) }}" class="pb-3 px-2 text-sm font-bold border-b-2 transition-all {{ $tab === 'history' ? 'border-sky text-sky-600' : 'border-transparent text-slate-400 hover:text-slate-600' }}">
                        <i class="fa-solid fa-clock-rotate-left mr-1.5"></i> Riwayat Transaksi
                    </a>
                </div>
                                ...
                                <tbody class="text-xs divide-y divide-slate-100">
                                    ...
                                    <tr>
                                        <td colspan="6" class="text-center py-10 text-slate-400 font-semibold text-xs">
                                            ...
                                            Belum ada transaksi pembayaran pada tab ini.
                                        </td>
                                    </tr>
                                    @endforelse
                                </tbody>
