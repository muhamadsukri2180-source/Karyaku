...
@php
    // Satu tempat untuk menerjemahkan status laporan
...
<div class="seller-page-head mb-4">
    ...
    <p class="small mb-0">Pantau laporan yang masuk terhadap toko Anda, dan lacak laporan yang Anda ajukan.</p>
</div>
            ...
            <form action="{{ route('penjual.laporan.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <div class="target-grid">
                        <div class="target-opt position-relative">
                            <input class="target-type" type="radio" name="target_type" id="tPengguna" value="pengguna" {{ old('target_type', 'pengguna') == 'pengguna' ? 'checked' : '' }}>
                            <label for="tPengguna"><i class="bi bi-person-fill"></i> Pengguna</label>
                        </div>
                        <div class="target-opt position-relative">
                            <input class="target-type" type="radio" name="target_type" id="tProduk" value="produk" {{ old('target_type') == 'produk' ? 'checked' : '' }}>
                            <label for="tProduk"><i class="bi bi-box-seam-fill"></i> Produk</label>
                        </div>
                        <div class="target-opt position-relative">
                            <input class="target-type" type="radio" name="target_type" id="tLain" value="lainnya" {{ old('target_type') == 'lainnya' ? 'checked' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    <select name="reason" id="reason" class="form-select @error('reason') is-invalid @enderror" required>
                        ...
                        @foreach ([
                            'Penipuan / tidak membayar pesanan',
                            'Perilaku tidak sopan',
                            'Penyalahgunaan chat/komplain',
                ...
                <div class="mb-3">
                    ...
                    <textarea name="description" id="description" rows="4" maxlength="1000"
                              ...
                              placeholder="Jelaskan kronologi dan bukti pelanggaran selengkap mungkin." required>{{ old('description') }}</textarea>
                ...
                <div class="d-flex align-items-start gap-2 p-3 rounded-3 mb-3" style="background:var(--bg-input); border:1px solid var(--border-color);">
                    ...
                    <small style="color:var(--text-muted); font-size:11.5px; line-height:1.6;">
                        Laporan ditinjau tim verifikator. Laporan palsu atau berulang tanpa dasar dapat berdampak pada akun Anda.
                    </small>
                </div>
                ...
                <div class="tab-pane fade {{ $tabAktif === 'masuk' ? 'show active' : '' }}" id="tab-masuk" role="tabpanel">
                    @if($incomingReports->isEmpty())
                        <div class="kk-empty">
                            ...
                            <h6 class="fw-bold mb-1" style="color:var(--text-dark);">Toko dan produk Anda bersih</h6>
                            <p class="small mb-0">Tidak ada laporan pelanggaran terhadap akun atau produk Anda.</p>
                        </div>
                    @else
                        <div class="d-flex align-items-start gap-2 p-3 rounded-3 mb-3" style="background:var(--bg-input); border:1px solid var(--border-color);">
                                    ...
                                    <div class="d-flex align-items-start justify-content-between gap-3">
                                        <div class="overflow-hidden">
                                            @if ($report->product)
                                                ...
                                                <span class="kk-chip mt-1" style="background:var(--primary-light); color:var(--primary);">Produk digital Anda</span>
                                            @else
                                                <div class="report-title"><i class="bi bi-person-exclamation me-1 text-danger"></i>Akun toko Anda dilaporkan</div>
                                                <span class="kk-chip mt-1" style="background:#f1f5f9; color:#475569;">Profil penjual</span>
                                            @endif
                                        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Tampilkan dropdown sesuai jenis target laporan
