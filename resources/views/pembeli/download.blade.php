    ...
    <div class="row g-3">
        @foreach ($orderItems as $item)
            ...
            <div class="col-md-6 col-lg-4">
                <div class="card-box p-3 h-100 d-flex flex-column justify-content-between border rounded-4 hover-shadow">
                    <div>
                        ...
                        <div class="p-2.5 bg-light rounded-3 small text-muted mb-3" style="font-size: 11px;">
                    ...
                    <div class="d-flex gap-2">
                        @if ($prod && $prod->file)
                            <a href="{{ route('pembeli.download.file', $item->id_order_item) }}" class="btn btn-primary btn-sm flex-fill fw-bold py-2 rounded-3 shadow-sm">
                                <i class="bi bi-download me-1"></i> Unduh File
                            </a>
                        @else
                            <button class="btn btn-secondary btn-sm flex-fill py-2 rounded-3" disabled>
                                <i class="bi bi-slash-circle me-1"></i> File Tidak Tersedia
                            </button>
                        @endif
                        @if($prod)
                            <a href="{{ route('pembeli.produk.detail', $prod->id_product) }}" class="btn btn-outline-secondary btn-sm px-2.5 py-2 rounded-3" title="Lihat Detail & Ulas">
