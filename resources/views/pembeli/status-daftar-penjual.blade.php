    ...
    <div class="card-box p-4 mb-4 border-success bg-success-subtle text-success-emphasis rounded-4">
        <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap">
            <div class="d-flex align-items-center gap-3">
                <div class="d-flex align-items-center justify-content-center bg-success text-white rounded-circle flex-shrink-0" style="width:52px;height:52px;">
...
@elseif (! $registration)
    ...
    <div class="card-box p-5 text-center text-muted">
        ...
        <a href="{{ route('pembeli.seller.registration.create') }}" class="btn btn-primary px-4 py-2.5 fw-bold rounded-3 shadow-sm">
...
@elseif ($status === 'pending')
    ...
    <div class="card-box p-4 mb-4 border-warning bg-warning-subtle text-warning-emphasis rounded-4">
        <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap">
            <div class="d-flex align-items-center gap-3">
                <div class="d-flex align-items-center justify-content-center bg-warning text-dark rounded-circle flex-shrink-0" style="width:52px;height:52px;">
            ...
            <form action="{{ route('pembeli.seller.registration.cancel') }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin membatalkan pendaftaran ini?')">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-outline-danger btn-sm fw-bold rounded-3 px-3 py-2">
...
@elseif ($status === 'rejected')
    {{-- STATUS DITOLAK --}}
    <div class="card-box p-4 mb-4 border-danger bg-danger-subtle text-danger-emphasis rounded-4">
        <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap">
            <div class="d-flex align-items-center gap-3">
                <div class="d-flex align-items-center justify-content-center bg-danger text-white rounded-circle flex-shrink-0" style="width:52px;height:52px;">
            ...
            <a href="{{ route('pembeli.seller.registration.create') }}" class="btn btn-danger fw-bold px-4 py-2 rounded-3 text-white flex-shrink-0">
                <i class="bi bi-arrow-repeat me-1"></i> Ajukan Ulang Pendaftaran
            </a>
        </div>
    </div>
@endif
...
@if ($registration)
    ...
    <div class="card-box p-4">
        <h6 class="fw-bold mb-4 border-bottom pb-3 text-dark d-flex align-items-center gap-2">
            <i class="bi bi-file-earmark-text-fill text-primary"></i> Rincian Berkas Pengajuan
        </h6>
        ...
        <div class="row g-4">
            ...
            <div class="col-md-6">
                ...
                <div class="mb-3">
                    <span class="text-muted small d-block">Nominal Biaya:</span>
        ...
        <div class="row g-4 border-top pt-4 mt-2">
            <div class="col-md-6">
                <span class="text-muted small d-block mb-2 fw-semibold">Foto KTP:</span>
                @if ($registration->identity_document)
                    <a href="{{ asset('storage/' . $registration->identity_document) }}" target="_blank">
                        <img src="{{ asset('storage/' . $registration->identity_document) }}" alt="Foto KTP" class="img-fluid rounded-3 border shadow-sm" style="max-height: 180px; object-fit: cover;">
                    </a>
                @else
                    <span class="text-muted small">Tidak ada foto KTP.</span>
                @endif
            </div>

            <div class="col-md-6">
                <span class="text-muted small d-block mb-2 fw-semibold">Bukti Pembayaran:</span>
                @if ($registration->payment_proof)
                    <a href="{{ asset('storage/' . $registration->payment_proof) }}" target="_blank">
                        <img src="{{ asset('storage/' . $registration->payment_proof) }}" alt="Bukti Pembayaran" class="img-fluid rounded-3 border shadow-sm" style="max-height: 180px; object-fit: cover;">
                    </a>
                @else
                    <span class="text-muted small">Tidak ada bukti transfer.</span>
                @endif
            </div>
        </div>
    </div>
